...
class BillPay extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (this.state.to_acct.match(/^[0-9]*$/gm) == null) {
      e.preventDefault();
      this.setState({
        errorToAcct: "Account number must contain only values 0-9 (inclusive)",
      });
    }
    ...
    if (this.state.routing_num.match(/^[0-9]*$/gm) == null) {
      e.preventDefault();
      this.setState({
        errorRouting: "Routing number must contain only values 0-9 (inclusive)",
      });
    }
    ...
    if (selectedDate < today) {
      e.preventDefault();
      this.setState({ errorDate: "Scheduled date cannot be in the past." });
    }
