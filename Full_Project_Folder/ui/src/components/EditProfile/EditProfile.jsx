...
class EditProfile extends React.Component {
    ...
    handleSubmit(e) {
        ...
        if (this.state.email.match(/^[a-z0-9]+[\._]?[a-z0-9]+[@]\w+[.]\w{2,3}$/gm) == null) {
            e.preventDefault();
            this.setState({
                err_email: 'Not valid email',
            });
        } else if (this.state.other_accts.some(acct => acct.email === this.state.email)) {
            e.preventDefault();
            this.setState({
        ...
        if (this.state.city.match(/^[a-zA-Z\u0080-\u024F\s\/\-\)\(\`\.\"\']{2,40}$/gm) == null) {
            e.preventDefault();
            this.setState({
                err_city: 'Not valid city: Only language based letters. Length b/w 2 and 40 chars allowed.',
            });
        } 
        ...
        if (this.state.region.match(/^[A-Z]{2,2}$/gm) == null) {
            e.preventDefault();
            this.setState({
                err_region: 'Not valid state: Only capital letters and length of 2',
            });
        } 
        ...
        if (this.state.phone_number.match(/^[0-9]{3,3}-[0-9]{3,3}-[0-9]{4,4}$/gm) == null) {
            e.preventDefault();
            this.setState({
                err_phone_number: 'Not valid phone number: Format should be XXX-XXX-XXXX and numbers only',
            });
        }

        const getAge = Math.floor((new Date() - new Date(this.state.birthday).getTime()) / 3.15576e+10)

        if (getAge <= 17 ) {
            this.setState({err_birthday:"You must be at least 18 years of old to join."})
        }
    }

    onSubmit = async (e) => {
        e.preventDefault()
        if ( (this.state.err_address == '') && 
            ...
            (this.state.err_zipcode == '')) {
                ...
                const response = await axiosInstance.post(`clients/${id}/edit_client/`, {
        ...
        return (
            <div className="EditProfile">
                        <div className="form">
                            ...
                            <button className="btn" type="submit" onSubmit={this.onSubmit} onClick={this.handleSubmit}>
                                Edit Profile
                            </button>
                            </form>
                        </div>
